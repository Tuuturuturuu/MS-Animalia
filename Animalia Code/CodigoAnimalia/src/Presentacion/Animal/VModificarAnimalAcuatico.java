...
public class VModificarAnimalAcuatico extends JFrame implements IGUI{
	TAnimal tAcuatico;

	public VModificarAnimalAcuatico(TAnimalAcuatico tAcuatico) {
		super("Modificar Animal Acuatico");
	...
	public void initGUI() {
		...
		JLabel msgIntroIDCabecera = ComponentsBuilder.createLabel(
				"Introduzca la temperatura y el tipo de agua del animal acuatico que quiere modificar ", 1, 10, 80,
				20, Color.BLACK);
		...
		botonAceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				VModificarAnimalAcuatico.this.setVisible(false);
				try {
					...
					ApplicationController.getInstance().manageRequest(
							new Context(Evento.MODIFICAR_ANIMAL_ACUATICO, animal));
	...
	public void actualizar(Context res) {
		// begin-user-code
		// TODO Auto-generated method stub
		if (res.getEvento() == Evento.MODIFICAR_ANIMAL_ACUATICO_OK) {
			ApplicationController.getInstance().manageRequest(new Context(Evento.V_CORRECTO, (int) res.getDatos()));
		} else if (res.getEvento() == Evento.MODIFICAR_ANIMAL_ACUATICO_KO) {
