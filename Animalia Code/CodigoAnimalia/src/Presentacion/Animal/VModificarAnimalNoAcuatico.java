...
public class VModificarAnimalNoAcuatico extends JFrame implements IGUI{
	TAnimal tAcuatico;

	public VModificarAnimalNoAcuatico(TAnimalNoAcuatico tNoAcuatico) {
		super("Modificar Animal No aquatico");
	...
	public void initGUI() {
		...
		JLabel msgIntroIDCabecera = ComponentsBuilder.createLabel(
				"Introduzca el numero de patas del animal no acuatico que quiere modificar ", 1, 10, 80,
				20, Color.BLACK);
		...
		botonAceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				VModificarAnimalNoAcuatico.this.setVisible(false);
				try {
					...
					ApplicationController.getInstance().manageRequest(new Context(Evento.MODIFICAR_ANIMAL_NO_ACUATICO, animal));
	...
	public void actualizar(Context res) {
		// begin-user-code
		// TODO Auto-generated method stub
		if (res.getEvento() == Evento.MODIFICAR_ANIMAL_NO_ACUATICO_OK) {			
			ApplicationController.getInstance().manageRequest(new Context (Evento.V_CORRECTO, (int) res.getDatos()));
		} else if (res.getEvento() == Evento.MODIFICAR_ANIMAL_NO_ACUATICO_KO) {
