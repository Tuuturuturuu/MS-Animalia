...
public class VModificarProductoJPA extends JFrame implements Presentacion.FactoriaVistas.IGUI {
	...
	private void initGUI() {
		...
		JLabel msgIntroIDCabecera = ComponentsBuilder.createLabel("Seleccione el id del Producto que desea modificar ",
		...
		botonAceptar.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				VModificarProductoJPA.this.setVisible(false);
				int tipo = -47548;
				TProducto prod = null;
				try {
					if (tipoProducto.getSelectedItem() == "Alimentacion") {
						...
						TAlimentacion alimentacion = new TAlimentacion(idT != null ? idT:0, nombreT != null ? nombreT : "",
								precioT != null ? precioT : 0, !stock.getText().isEmpty() ? stockT : 0,
								tType != null ? tType : "", true, idMarcaT != null ? idMarcaT : 0);

						prod = alimentacion;
						ApplicationController.getInstance()
								.manageRequest(new Context(Evento.MODIFICAR_PRODUCTO, prod));
						...
						TMerchandising merch = new TMerchandising(idT != null ? idT:0, nombreT != null ? nombreT : "",
								precioT != null ? precioT : 0, !stock.getText().isEmpty() ? stockT : 0,
	...
	public void actualizar(Context res) {
		...
		if (res.getEvento() == Evento.MODIFICAR_PRODUCTO_OK) {
			ApplicationController.getInstance().manageRequest(new Context(Evento.V_CORRECTO, (int) res.getDatos()));
		} else if (res.getEvento() == Evento.MODIFICAR_PRODUCTO_KO) {
